...
import { Check, Copy, Home, RefreshCw, X, Crown, Target, Shuffle, Trophy, AlertTriangle } from "lucide-react"
...
export default function GamePage() {
  ...
  const [selectedRole, setSelectedRole] = useState<"playerOne" | "playerTwo" | null>(null)
  ...
  useEffect(() => {
    if (
      gameData &&
      players.length > 0 &&
      (!gameData.playerOneBoard || Object.keys(gameData.playerOneBoard).length === 0)
    ) {
      const gameRef = ref(database, `games/${id}`)
  ...
  const selectRole = (role: "playerOne" | "playerTwo") => {
    ...
    if (bothJoined) {
      if (gameData?.targetMode === "random") {
        ...
        const availablePlayers = players.filter((p) => p.id !== gameData.playerOneId && p.id !== gameData.playerTwoId)
  ...
  const togglePlayerCrossed = (playerId: string) => {
    ...
    if (playerId === gameData.playerOneId || playerId === gameData.playerTwoId) {
      toast({
  ...
  const eliminatePlayer = (playerId: string) => {
    ...
    const currentBoard = gameData[boardKey as keyof GameData] as Record<string, { crossed: boolean }>
    const newBoard = {
      ...currentBoard,
  ...
  const makeGuess = () => {
    ...
    const myTargetId = selectedRole === "playerOne" ? gameData.playerTwoTargetId : gameData.playerOneTargetId
  ...
  const nextTurn = () => {
    ...
    // Switch turns
    const nextTurn = gameData.currentTurn === "playerOne" ? "playerTwo" : "playerOne"
  ...
  const getMyBoard = () => {
    if (!gameData || !selectedRole) return {}
    return gameData[`${selectedRole}Board` as keyof GameData] as Record<string, { crossed: boolean }>
  }
  ...
  const getRemainingPlayers = () => {
    ...
    return players.filter(
      (player) =>
        !myBoard[player.id]?.crossed && player.id !== gameData?.playerOneId && player.id !== gameData?.playerTwoId,
    )
  }
  ...
  const getCrossedCount = () => {
    ...
    const eligiblePlayers = players.filter(
      (player) => player.id !== gameData?.playerOneId && player.id !== gameData?.playerTwoId,
    )
    return eligiblePlayers.filter((player) => myBoard[player.id]?.crossed).length
  }

  const getOpponent = () => {
    if (!gameData || !selectedRole) return null
    const opponentId = selectedRole === "playerOne" ? gameData.playerTwoId : gameData.playerOneId
    return getPlayer(opponentId)
  }

  const getMyPlayer = () => {
    if (!gameData || !selectedRole) return null
    const myId = selectedRole === "playerOne" ? gameData.playerOneId : gameData.playerTwoId
    return getPlayer(myId)
  }

  const getMyTarget = () => {
    if (!gameData || !selectedRole) return null
    const targetId = selectedRole === "playerOne" ? gameData.playerTwoTargetId : gameData.playerOneTargetId
    return targetId ? getPlayer(targetId) : null
  }

  const getOpponentTarget = () => {
    if (!gameData || !selectedRole) return null
    const targetId = selectedRole === "playerOne" ? gameData.playerOneTargetId : gameData.playerTwoTargetId
    return targetId ? getPlayer(targetId) : null
  }
  ...
  const getAvailableTargets = () => {
    if (!gameData) return []
    const otherRole = selectedRole === "playerOne" ? "playerTwo" : "playerOne"
    const otherTargetId = gameData[`${otherRole}TargetId` as keyof GameData] as string

    return players.filter(
      (player) =>
        player.id !== gameData.playerOneId && player.id !== gameData.playerTwoId && player.id !== otherTargetId,
    )
  }
              ...
              <div className="flex items-center space-x-4">
                <div className="text-center">
                  <Avatar className="h-12 w-12 mx-auto mb-2">
                    {playerOne?.imageUrl ? (
                      <AvatarImage src={playerOne.imageUrl || "/placeholder.svg"} alt={playerOne.name} />
                    ) : (
                      <AvatarFallback>{playerOne?.name.substring(0, 2).toUpperCase()}</AvatarFallback>
                    )}
                  </Avatar>
                ...
                <div className="text-center">
                  <Avatar className="h-12 w-12 mx-auto mb-2">
                    {playerTwo?.imageUrl ? (
                      <AvatarImage src={playerTwo.imageUrl || "/placeholder.svg"} alt={playerTwo.name} />
                    ) : (
                      <AvatarFallback>{playerTwo?.name.substring(0, 2).toUpperCase()}</AvatarFallback>
                    )}
                  </Avatar>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Button
                ...
                disabled={gameData.playerOneJoined}
                className="h-20 text-lg"
              >
                I am {playerOne?.name} (Player One)
              </Button>
              <Button
                onClick={() => selectRole("playerTwo")}
                disabled={gameData.playerTwoJoined}
                className="h-20 text-lg"
              >
                I am {playerTwo?.name} (Player Two)
              </Button>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="space-y-4">
              <p className="text-muted-foreground">
                You are {myPlayer?.name}. Choose a target for {opponent?.name} to find by elimination.
              </p>

              {!hasMyTargetBeenSelected() ? (
                <>
                  <div className="space-y-2">
                    <Label htmlFor="target">Select Target for {opponent?.name}</Label>
                  ...
                  <Button onClick={selectTarget} disabled={!selectedTarget} className="w-full">
                ...
                <div className="text-center p-4 bg-green-50 dark:bg-green-950/20 rounded-lg">
                  ...
                  <p className="text-green-700 dark:text-green-300 font-medium">Target Selected!</p>
        ...
        <Card className="mb-6">
          <CardContent className="p-8 text-center">
            <div className="mb-6">
              ...
              <h2 className={`text-3xl font-bold mb-2 ${isWinner ? "text-green-600" : "text-red-600"}`}>
                {isWinner ? "🎉 You Won!" : "😞 You Lost!"}
              </h2>
              <p className="text-lg text-muted-foreground mb-4">{winner} guessed correctly!</p>
            </div>
              ...
              <div className="text-center">
                ...
                <Avatar className="h-16 w-16 mx-auto mb-2">
                  {guessedPlayer?.imageUrl ? (
                    <AvatarImage src={guessedPlayer.imageUrl || "/placeholder.svg"} alt={guessedPlayer.name} />
                  ) : (
                    <AvatarFallback>{guessedPlayer?.name.substring(0, 2).toUpperCase()}</AvatarFallback>
                  )}
                </Avatar>
              ...
              <div className="text-center">
                ...
                <Avatar className="h-16 w-16 mx-auto mb-2">
                  {actualTarget?.imageUrl ? (
                    <AvatarImage src={actualTarget.imageUrl || "/placeholder.svg"} alt={actualTarget.name} />
                  ) : (
                    <AvatarFallback>{actualTarget?.name.substring(0, 2).toUpperCase()}</AvatarFallback>
                  )}
                </Avatar>
  ...
  return (
    <div className="container mx-auto p-4 md:p-8">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">Elimination Game</h1>
        <div className="flex space-x-2">
          <Button variant="outline" onClick={copyGameId}>
            {copied ? <Check className="h-4 w-4 mr-2" /> : <Copy className="h-4 w-4 mr-2" />}
            {id.substring(0, 8)}...
        ...
        <Card className="mb-6 border-orange-200 bg-orange-50 dark:bg-orange-950/20">
          <CardContent className="p-4">
            <div className="flex items-start space-x-3">
              ...
              <div>
                <h3 className="font-semibold text-orange-800 dark:text-orange-200 mb-1">How to Play</h3>
                <p className="text-sm text-orange-700 dark:text-orange-300">
                  Cross off players to eliminate them. When only one player remains, you'll be asked if that's your
                  final guess for your opponent's target. Guess correctly to win!
                </p>
              </div>
            </div>
          </CardContent>
      ...
      <div className="flex flex-col lg:flex-row gap-4 mb-6">
        {/* Opponent Card - Left side */}
        <Card className="w-full lg:w-80 border-2 border-primary/20 bg-gradient-to-br from-primary/5 to-primary/10">
          <CardContent className="p-4">
            <div className="flex items-center space-x-3">
              <div className="relative">
                <Avatar className="h-12 w-12 border-2 border-primary/20">
                  {opponent?.imageUrl ? (
                    <AvatarImage src={opponent.imageUrl || "/placeholder.svg"} alt={opponent.name} />
                  ) : (
                ...
                <div className="flex items-center space-x-2 mt-1">
                  <div
                    className={`h-2 w-2 rounded-full ${
                      gameData.currentTurn === (selectedRole === "playerOne" ? "playerTwo" : "playerOne")
            ...
            <CardContent className="p-4">
              <div className="flex items-center space-x-3">
                <div className="relative">
                  <Avatar className="h-12 w-12 border-4 border-purple-400">
                    {opponentTarget.imageUrl ? (
                      <AvatarImage src={opponentTarget.imageUrl || "/placeholder.svg"} alt={opponentTarget.name} />
                    ) : (
                      <AvatarFallback className="text-sm font-bold">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-purple-700 dark:text-purple-300 font-medium">👑 Opponent's Target</p>
                  <p className="text-xs text-purple-600 dark:text-purple-400">They're looking for this player</p>
                </div>
              </div>
            </CardContent>
          </Card>
      ...
      <div className="mb-6 flex justify-between items-center">
        <div>
          <h2 className="text-xl font-semibold">
            You are: {myPlayer?.name} ({selectedRole === "playerOne" ? "Player One" : "Player Two"})
          </h2>
          <p className="text-muted-foreground">
            {gameData.currentTurn === selectedRole ? "🟢 Your turn" : "⏳ Waiting for opponent"}
          </p>
          <p className="text-sm text-muted-foreground">
            Eliminated: {getCrossedCount()} players • Remaining: {remainingPlayers.length} players
          </p>
        </div>
        <div className="flex space-x-2">
          <Button onClick={nextTurn} disabled={gameData.currentTurn !== selectedRole}>
            End Turn
          </Button>
      ...
      <div className="grid grid-cols-5 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-2 sm:gap-3 md:gap-4">
        {players.map((player) => {
          ...
          const isGamePlayer = player.id === gameData.playerOneId || player.id === gameData.playerTwoId
              ...
              <CardContent className="p-1 sm:p-2 md:p-4 flex flex-col items-center justify-center text-center relative">
                ...
                {isCrossed && (
                  <div className="absolute inset-0 flex items-center justify-center">
                    <X className="h-6 w-6 sm:h-8 sm:w-8 md:h-12 md:w-12 animate-pulse text-red-500" strokeWidth={3} />
                  </div>
                )}
      ...
      <AlertDialog open={showGuessDialog} onOpenChange={setShowGuessDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              You're about to eliminate another player, leaving only one remaining. Are you sure{" "}
          ...
          <div className="flex justify-center my-4">
            <div className="text-center">
              <Avatar className="h-16 w-16 mx-auto mb-2">
                {finalGuess?.imageUrl ? (
                  <AvatarImage src={finalGuess.imageUrl || "/placeholder.svg"} alt={finalGuess.name} />
                ) : (
                  <AvatarFallback className="text-lg">{finalGuess?.name.substring(0, 2).toUpperCase()}</AvatarFallback>
                )}
              </Avatar>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setFinalGuess(null)}>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={makeGuess} className="bg-green-600 hover:bg-green-700">
